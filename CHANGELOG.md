...
From this release on, the package major version matches the highest Laravel major version it supports, so `^13.0`
covers Laravel 13.x and 12.x. Development happens on `master`; the `v13.x` branch tracks it and carries the released
state.
...
- `laravel/framework` is constrained to `^12.61.1|^13.0`, which excludes the framework versions affected by the
  published CRLF injection and signed URL advisories.
...
- The `Update Changelog` workflow checks out with `CHANGELOG_TOKEN` when that secret exists, so the commit is not
  rejected once branch protection is enabled on `master`.

### Fixed

- Package discovery registered the facade under the alias `Woocommerce`, a leftover from another package. The alias is
  now `Genderize`, which is what the README has always documented.

### Removed

- Support for Laravel 11.x and 10.x. Both are past their security support window, and Composer refuses to install them
  because of published security advisories.
- Support for PHP 8.2 and 8.1. Laravel 12 still runs on PHP 8.2, but that version reaches end of life in December 2026
  and Laravel 13 requires PHP 8.3 or later.
